...
async function getProgressionStats(context: ToolContext) {
  ...
  const nextSkillId =
    currentIndex < allSkillIds.length - 1
      ? allSkillIds[currentIndex + 1]
      : null;
...
async function getHindrancePatterns(context: ToolContext, limit?: number) {
  ...
  if (entries.length === 0) {
    return {
      message:
        "No hindrance entries found - the user may be new or hasn't logged struggles yet",
      patterns: [],
    };
  }
